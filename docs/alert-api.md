...
名单缺失接管补丁（发布状态见[1.0.74 台账](release-1.0.74.md)）仍使用现有 `alert.updated`/Bootstrap：主来源长期只报图标、
备用来源有新鲜完整 OCR 名单时，按服务端保护条件整体切换主来源，不合并节点名单。
接管不制造一次“清空再来敌”；消费者按同一星系最新权威状态更新人员信息。
...
机器人按角色 ID 集合及名单修订去重，人员新增、移除及空名单更正统一显示“⚠️ 敌对事件”；同 ID 改名不重复告警。
