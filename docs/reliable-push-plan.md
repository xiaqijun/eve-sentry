...
文中标为“现行”或列为已勾选的内容表示截至 v1.0.67 已实现；标为“目标”或未勾选的内容
仍是后续设计，不能当作生产现状。
...
断流重连。`9f40b39` 对应地缩短全局锁持有时间，把数据库/ESI 等待移出锁外，并让重连从
持久状态序号恢复；服务端和机器人部署后的健康检查均通过，尚需真实事件完成最终验收。
