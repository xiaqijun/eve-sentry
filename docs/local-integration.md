...
点击 `停止监控` 后，界面会立即停止上报并异步等待 OCR 工作线程退出，不会在 Qt
主线程中同步等待。旧工作线程完全退出前，`开始监控` 按钮保持禁用，避免同一窗口
出现两组采集线程。停止监控的本地星系会立即从预警浮窗移除，不生成“清空”通知。
...
浮窗为每个在线监控节点所在星系显示一个紧凑方框：有敌对时为红色并实时显示人数，
安全时显示绿色和 `敌 0`；节点上线、切换星系、停止监控或离线后会通过 SSE bootstrap
实时同步。停止监控或离线的安全节点会立即移除；没有可显示节点时，浮窗收缩到仅含
标题和连接状态的高度，标题仍固定在顶部，不构造展示数据。
