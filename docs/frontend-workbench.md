...
- 已新增 `/reports` 敌对来袭报表，按服务端历史告警统计时间趋势、星系和目标排行。
...
`/reports` 是独立历史统计页面，通过 `/api/alerts` 读取服务端已经生成的
`ThreatEvent`。报表不把 OCR 当前可见名单、active intel 行或客户端 heartbeat
直接计为“来袭”；只有服务端完成敌对分类并生成告警后才进入统计。
...
- 独立敌对：按角色名去重后的目标数量。
- 涉及星系：发生过告警的星系数量。
- 高危批次：`critical` 和 `high` 告警数量。
- 日均来袭：按所选时间范围折算的平均批次。

页面同时展示风险等级分布、星系来袭排行、高频敌对目标和最近 12 条来袭记录。
数据每 60 秒自动刷新，也可以手动刷新；工作台左侧提供“来袭报表”入口。
...
- `/` 指向 `frontend/dist/`
- `/reports` 由 React Router 渲染敌对来袭报表，OpenResty/Nginx 需继续回退到 `index.html`。
