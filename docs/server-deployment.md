...
它会复用 `/etc/eve-sentry/eve-sentry.env` 里的这些配置:

- `EVE_SENTRY_SERVER_ESI_CLIENT_ID`
- `EVE_SENTRY_SERVER_ESI_REDIRECT_URI`
- `EVE_SENTRY_SERVER_ESI_TOKEN_FILE`

无浏览器环境可额外加 `--esi-no-browser`，再手工打开终端打印的授权 URL。
