...
4. 密钥有效后，Listener 检测作为独立后台任务运行。客户端自动定位 EVE Chatlogs，并扫描全部历史日志。
   没有发现 `Listener` 不算校验失败，也不阻止启动。
5. 发现 `Listener` 后，客户端调用 `/api/v1/client/identity-checks`，只提交角色名和客户端 ID。
6. 服务端持久化幂等任务并立即确认；后台 worker 再通过 ESI 解析角色 ID、军团和联盟。
7. 角色属于任一允许军团，或角色 ID 位于该用户白名单时继续使用；否则触发风控。
8. 已配置密钥的验证长期有效；后续只有新增角色、规则变化或管理员操作会重新判定角色身份。

仅在已配置且验证通过设备密钥时，客户端每 10 秒重新确认当前活跃的 Chatlogs 目录，
并只查找新增日志文件，不重复扫描
已处理文件的内容。新文件尚未写出 `Listener` 时不会标记完成。相同密钥和角色集合复用
...
后会重新扫描历史日志，并在发现 `Listener` 后提交风控校验；未配置密钥的客户端仍由
服务端认证模式决定能否访问。
