...
名单缺失接管（1.0.74 配套服务端，发布状态见[台账](release-1.0.74.md)）：主来源连续检测到敌对，但当前画面没有非空常规 OCR
名单，持续至少 15 秒且累计至少 3 个独立采集帧时，允许完整名单备用来源接管。
时间以服务端接收为准，重复 sequence 不累计；归零、重新开始采集会话或收到当前画面的
非空名单会重置等待。仅缺少 ESI 解析结果不等同于缺少 OCR，不以 ESI 延迟触发本规则。
...
新客户端将空 OCR 通过现有 `/ocr/snapshot` 的 `names:[]` 和 capture 上报，不新增质量枚举。
...
1.0.72 及旧服务端可能仍发送旧人数；新消费端同样丢弃整个未知状态，不将旧值显示为“上次”。
停止/离线节点不展示为实时监控源；同星系仍有有效备用时继续使用服务端选定结果。
...
需要可靠补发的消费者必须保存并推进事件游标，不能在每次重连时反复拉取历史事件。服务端优先读取
`Last-Event-ID`：
...
连接内仍保存事件检查点且不允许合成 ID 降级覆盖，告警 ID 去重记录保留；此策略仅适用于实时预警视图，
...
消失。失联造成的敌情失效也会移除，但不表示确认安全。接口不返回角色、客户端、评分或其他内部告警信息。需要实时预警消息、清空事件和
