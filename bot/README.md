...
新版主动预警消费服务端[统一星系状态](../docs/system-current-state.md)：未知采集只推进游标、不重复来敌或假清空；
名单更新采用最新人数，不保留波次最大值，人数单独变化不重复发送相同人员表。按需 OCR 查询不参与自动主节点选择。
...
2026-09-12 待发布兼容：服务端人员档案 `on` 模式按军团/联盟关系计算敌我，
不保留个人声望例外；机器人消费服务端结果，不另查个人声望。
`esi_organization_pending` 表示关系尚不确定，不得回退到旧的扁平声望值或当作默认 0；
该保护不改变视觉敌对人数驱动的来敌、清空流程。生产发布状态见部署记录。
