...
test("defaults Release requests to the monorepo", async () => {
  ...
  try {
    const request = new Request("https://download.example/latest.json");
    const response = await worker.fetch(request, {});

    assert.equal(response.status, 200);
    assert.match(target, /xiaqijun\/eve-sentry\/releases\/latest/);
  } finally {
