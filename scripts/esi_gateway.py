    ...
    def health(self) -> dict[str, Any]:
        with self.lock:
            ...
            return {
                ...
                "cache_hit_rate": round(self.cache_hits / max(1, self.cache_hits + total), 4),
                "rate_limit_per_second": round(1.0 / self.min_interval, 2),
                "latency_ms": {
                    "last": round(self.last_latency * 1000, 2),
                    "average": round(self.total_latency / max(1, self.requests) * 1000, 2),
                },
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            data, cache = self.server.state.fetch(key, lambda: getattr(self.server.state.client, method_name)(entity_id))
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if self.path.rstrip("/") == "/v1/universe/ids":
                ...
                key = "POST:ids:" + hashlib.sha256(json.dumps(payload, sort_keys=True).encode()).hexdigest()
            else:
                ...
                key = "POST:names:" + hashlib.sha256(json.dumps(ids).encode()).hexdigest()
            data, cache = self.server.state.fetch(key, loader)
