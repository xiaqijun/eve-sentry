...
def test_reliable_uploader_prioritizes_state_heartbeat_before_presence(tmp_path):
    calls = []
    ...
    try:
        with manager._condition:
            ...
            manager.submit_heartbeat(
                {"client_id": "detector-1", "status": "idle"},
                {"kind": "heartbeat", "task_key": "heartbeat:offline"},
            )

        assert wait_until(lambda: len(calls) == 2)
        assert calls == ["heartbeat", "presence"]
    finally:
        manager.shutdown()
...
def test_reliable_uploader_rate_limits_bursty_heartbeat_submissions(tmp_path):
    calls = []
    ...
    try:
        ...
        time.sleep(0.2)
        assert len(calls) == 1
        assert manager.pending_heartbeat_count() == 1
    finally:
        manager.shutdown()
