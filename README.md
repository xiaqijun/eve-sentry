...
面向 EVE Online 国际服 Tranquility 的军团战前情报工具。机器人接收最多 30 个角色名，结合 CCP ESI 与 zKillboard 的公开数据，生成可解释的舰队构成、小队规模、活跃时段和关键成员画像。
...
预警群在敌对进入新星系或发生星系移动时，会按当前已识别人员自动提交一次分析任务，
分析完成后以主动图片消息发送。自动分析与手动分析共享群级并发限制；没有可识别人员时
不会提交任务，避免对仅有红色图标的状态发起无效请求。
